...
#https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW.nc?CRW_BAA%5B(2010-01-01T12:00:00Z):1:(2025-02-08T12:00:00Z)%5D%5B(-14.645754):1:(-14.700424)%5D%5B(145.448641):1:(145.454105)%5D,CRW_BAA_7D_MAX%5B(2010-01-01T12:00:00Z):1:(2025-02-08T12:00:00Z)%5D%5B(-14.645754):1:(-14.700424)%5D%5B(145.448641):1:(145.454105)%5D,CRW_DHW%5B(2010-01-01T12:00:00Z):1:(2025-02-08T12:00:00Z)%5D%5B(-14.645754):1:(-14.700424)%5D%5B(145.448641):1:(145.454105)%5D,CRW_HOTSPOT%5B(2010-01-01T12:00:00Z):1:(2025-02-08T12:00:00Z)%5D%5B(-14.645754):1:(-14.700424)%5D%5B(145.448641):1:(145.454105)%5D,CRW_SST%5B(2010-01-01T12:00:00Z):1:(2025-02-08T12:00:00Z)%5D%5B(-14.645754):1:(-14.700424)%5D%5B(145.448641):1:(145.454105)%5D,CRW_SSTANOMALY%5B(2010-01-01T12:00:00Z):1:(2025-02-08T12:00:00Z)%5D%5B(-14.645754):1:(-14.700424)%5D%5B(145.448641):1:(145.454105)%5D
...
#gbr_shapefile <- download_gbr_spatial(return = "base")
gbr_shapefile <- st_read("data/zipfolder/Great_Barrier_Reef_Features.shp")  

  

# extract and tidy to sf
lizard_shape <- gbr_shapefile |> 
  filter(str_detect(GBR_NAME, "Lizard"))
...
# read in SST
northpointDHW<- rast("data/dhw_5km_3231_0c35_4219_U1739212814032.nc")

filtered_northpointDHW <- northpointDHW["CRW_DHW"]#, varnames(northpointDHW)))

# Filter layers between the specified dates
filtered_northpointDHW <- filtered_northpointDHW[[which(
   time(filtered_northpointDHW) >= as.Date("2024-03-10") &
   time(filtered_northpointDHW) <= as.Date("2024-03-11")
)]]



ggplot() + theme_bw() +
  geom_spatraster(data = filtered_northpointDHW) +
  scale_fill_gradientn(colors = (RColorBrewer::brewer.pal(9, "Reds")), name="DHW", limits=c(4,8)) +
  ...
  ggtitle("10th Feb 2025 (NOAA DHW)") + xlab("") + ylab("")
...
Create `calculate_spatial_average` function that takes each raster input and computes the average DHW value for each timestep across the input data

```{r, fig.width=8, fig.height=3, message=FALSE, warning=FALSE}


# Function to compute spatial average for each layer
calculate_spatial_average <- function(raster) {
...
ggplot() + theme_bw() +
  scale_x_continuous(limits=c(2009, 2024.5), breaks=seq(2010, 2024,2)) +
...
#  filter(year > 2014) %>%
  ...
  scale_color_manual(name = "Year", values =colorRampPalette(RColorBrewer::brewer.pal(11, "RdBu"))(15)) +
...
# get climatology
mmm_val <- terra::rast("data/GBR_ct5km_MMM_v3.1.nc") |> 
  crop(lizard_shape) |> 
  values() |> 
  as.numeric()
...
CC
'
output <- wrap_plots(A = maxdhw, B = timeseries_dhw, C = sst, widths=c(1,1),  heights = c(4, 5), design = layout)

output


ggsave("outputs/layout.pdf", plot = output, width = 10, height = 8, device = "pdf")
ggsave("outputs/layout.png", plot = output, width = 10, height = 8, dpi = 300, device = "png")
